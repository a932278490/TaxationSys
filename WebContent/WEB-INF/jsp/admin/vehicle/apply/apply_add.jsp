...
function addOption(obj){
	...
	for(j=0;j<obj.length;j++){
		if(obj[j].name=="${loginStuff.name}" ){
			str+="<option selected=\"selected\" value=\""+obj[j].name+"\">"+obj[j].name+"</option>";
		}else{
...
function addPeople(count){
	...
	for(i=0;i<count;i++){
		$("#pname").append("<div class=\"cityleft\"> <select id=\"pvalue\" class=\"select2\" style=\"width: 167px;\" name=\"pname\">"+str+"</select> </div>");
	}
...
<div class="place">
    ...
    <div class="itab">
  	<ul> 
    <li><a href="#tab1" class="selected">填写出车申请</a></li> 
    ...
    <div class="formtext">Hi，<b>${loginStuff.name}</b>，欢迎您填写出车申请！</div>
    
    
    <form action="admin/vehicle/apply/save" method="post">
    <input type="hidden" name="stuffId" value="${loginStuff.id}"/>
	<input type="hidden" name="deptId" value="${loginStuff.deptId }" />
    <ul class="forminfo">
    <li><label>出车目的地<b>*</b></label><input name="destination" type="text" class="dfinput" value="请填写地名"  style="width:518px;"/></li>

...
<div class="pd-20">
	...
	<a href="javascript:;" onclick="cancelDistribute(${vehicleId})" class="btn btn-danger radius">
...
function cancelDistribute(vehicleId){
	layer.confirm('确认取消分配到该车吗？',function(index){
		var chks=$("input:checked");//获取所有选中的checkbox,chks是一个元素数组
		for(var i=0;i<chks.length;i++){
			$.ajax({
				url: "admin/vehicle/apply/cancelDistribute",
				type: 'POST',
				data: "id="+chks[i].value+"&vehicleId="+vehicleId,
				async : false,
				success:function(data){
					if (data.status) {
						layer.msg('已取消分配!',{icon:1,time:2000});
					}else{
